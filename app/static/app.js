(() => {
  ...
  function totals(){return readyRows().reduce((a,r)=>{const c=calcRow(r);for(const k of ['meters','fabricCost','confectionCost','installationCost','base','benefit','total'])a[k]+=c[k];return a},{meters:0,fabricCost:0,confectionCost:0,installationCost:0,base:0,benefit:0,total:0})}
  ...
  function validateAll(){
    ...
    state.rows.forEach(r=>{
      ...
      if(fabricWidth>0&&c.cutWidth>0&&c.cutWidth>fabricWidth-0.1)issues.push({level:'warning',text:c.cutWidth>fabricWidth?`Ancho de corte ${fmt(c.cutWidth)} m excede el ancho de tela (${fmt(fabricWidth)} m)`:`Ancho de corte ${fmt(c.cutWidth)} m a menos de 10 cm del ancho de tela (${fmt(fabricWidth)} m)`});
      if(fabricWidth>0&&c.cutHeight>0&&c.cutHeight>fabricWidth)issues.push({level:'warning',text:`Alto de corte ${fmt(c.cutHeight)} m no cabe en el ancho de tela (${fmt(fabricWidth)} m)`});
  ...
  function renderConfection(){
    ...
    q('#confectionPages').innerHTML=chunks.map((chunk,idx)=>{
      ...
      const lines=chunk.map(r=>{
        const c=calcRow(r);
        total+=c.meters;
        // Un ancho de corte por paño/hoja: 2 columnas si hay 2 hojas; con hoja única «Ancho 2» queda vacía.
        const cut2 = c.sheets > 1 ? `<td class="key-cut">${fmt(c.cutWidth)}</td>` : '<td></td>';
        return `<tr><td class="left"><b>${esc(rowLabel(r))}</b></td><td class="key-cut">${fmt(c.cutWidth)}</td>${cut2}<td>${fmt(c.finalHeight)}</td><td><b>${fmt(c.gather)}</b></td><td>${c.sheets}</td><td>${esc(state.project.fabricType)}</td><td>${fmt(c.metersPerSheet)}</td><td class="key-cut"><b>${fmt(c.meters)}</b></td><td class="checkbox-cell"></td></tr>`;
      }).join('');
      ...
      const totalLabel=sourceRows.reduce((a,x)=>a+calcRow(x).meters,0);
  ...
  function renderCutTable(){
    ...
    const detailRow=({r,c})=>`<tr><td class="room">${esc(rowLabel(r))}</td><td class="dim">${fmt(c.width)} × ${fmt(c.height)}</td><td class="dim">${fmt(c.finalHeight)}</td><td class="key-cut">${fmt(c.cutWidth)} × ${fmt(c.cutHeight)} m</td><td class="num"><b>${fmt(c.gather)}</b></td><td><span class="cut-badge">${c.sheets}</span></td><td class="dim num">${fmt(c.metersPerSheet)}</td><td class="num"><b>${fmt(c.meters)}</b></td></tr>`;
  ...
  function renderCutSummary(){
    ...
    const groups=groupedCuts(),all=allGroupedCuts(),totalWindows=all.reduce((a,g)=>a+g.windows,0),totalCuts=all.reduce((a,g)=>a+g.cuts,0),totalMeters=totals().meters;
  ...
  function railRows(){return readyRows().map(r=>{const width=num(r.width),final=width-num(state.project.railDeduction),supports=Math.max(0,Math.ceil(width/.5));return {r,width,final,supports,dark:final/2+.1}})}
  ...
  function exportXLSX(){
    ...
    rows.forEach(r=>{const c=calcRow(r);confeccion.push([rowLabel(r),c.width,c.height,c.panelWidth,c.finalHeight,c.cutWidth,c.cutHeight,c.sheets,c.metersPerSheet,c.meters,STATUS[r.status]?.label])});const ws2=XLSX.utils.aoa_to_sheet(confeccion);ws2['!cols']=[{wch:15},{wch:13},{wch:13},{wch:15},{wch:13},{wch:13},{wch:13},{wch:10},{wch:11},{wch:14},{wch:18}];XLSX.utils.book_append_sheet(wb,ws2,'CONFECCION');
    ...
    const rails=[['RIELES'],['Obra',p.hotel],[],['Identificador','Ancho hueco','Medida final','Soportes']];railRows().forEach(x=>rails.push([rowLabel(x.r),x.width,x.final,x.supports]));const ws4=XLSX.utils.aoa_to_sheet(rails);ws4['!cols']=[{wch:16},{wch:15},{wch:15},{wch:12}];XLSX.utils.book_append_sheet(wb,ws4,'RIELES');
